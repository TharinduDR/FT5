...
for i in range(FOLDS):
    german_data = pd.read_csv("https://raw.githubusercontent.com/TharinduDR/DeepOffense/master/examples/german/data/GermEval21_Toxic_Train.csv")
    german_data['label'] = german_data['Sub1_Toxic'].replace([0, 1], ['TOX', 'NOT'])
    german_data = german_data[["comment_text", "label"]]

    german_train, german_test = train_test_split(german_data, test_size=0.2, random_state=SEED)

    german_train["prefix"] = "cctk"
    german_train = german_train.rename(columns={'comment_text': 'input_text', 'label': 'target_text'})
    german_train = german_train[["prefix", "input_text", "target_text"]]

    train_df, eval_df = train_test_split(german_train, test_size=0.2, random_state=SEED * i)
    ...
    model_name_prefix = "German" + model_name
    ...
    for index, row in german_test.iterrows():
        test_list.append("cctk: " + row['comment_text'])
    ...
    macro_f1, weighted_f1 = sentence_label_evaluation(preds, german_test["label"].tolist())
